...
const App: React.FC = () => {
  ...
  const cleanText = (text: string) => {
    // Aggressive tag stripping for the "Unsensor" / Tagless requirement
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          speechConfig: {
            voiceConfig: { 
              prebuiltVoiceConfig: { voiceName: 'Puck' } // Puck has a more nuanced, "talkative" quality
            }, 
          },
    ...
    <div className="flex flex-col h-screen w-full bg-[#050505] text-white selection:bg-blue-500/30 overflow-hidden font-sans">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[140%] h-[140%] rounded-full transition-all duration-1000 blur-[140px] ${
          isSpeaking ? 'bg-blue-600/15 opacity-100 scale-110' : 
      ...
      <header className="z-20 px-6 pt-10 pb-4 flex justify-between items-center shrink-0">
        ...
        <div className="px-4 py-1.5 rounded-full border border-white/10 bg-black/60 backdrop-blur-2xl flex items-center gap-3">
          <div className={`w-2.5 h-2.5 rounded-full ${status === ConnectionStatus.CONNECTED ? 'bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,1)] animate-pulse' : 'bg-zinc-800'}`} />
      ...
      <main className="flex-1 flex flex-col relative z-10 px-6 overflow-hidden">
        <div ref={scrollRef} className="flex-1 flex flex-col justify-center space-y-8 py-12 scrollbar-hide overflow-y-auto">
          <div className="space-y-4 opacity-20 mask-fade-top pointer-events-none transition-opacity duration-1000">
            {history.map((t) => (
              <div key={t.id} className={`flex flex-col ${t.sender === 'user' ? 'items-end' : 'items-start'}`}>
                <p className={`max-w-[85%] text-lg font-light italic leading-tight ${t.sender === 'user' ? 'text-zinc-400' : 'text-blue-200'}`}>
                  {t.text}
                </p>
              </div>
            ))}
          ...
          <div className="min-h-[160px] flex flex-col justify-center">
            {activeTranscription.text ? (
              <div className={`flex flex-col transition-all duration-300 ${activeTranscription.sender === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`max-w-full text-3xl md:text-5xl font-bold leading-[1.05] tracking-tighter text-center ${
                  activeTranscription.sender === 'user' ? 'text-zinc-400' : 'text-white'
                }`}>
                  {activeTranscription.text}
                  <span className="inline-block w-2 h-10 bg-blue-500 ml-3 animate-pulse align-middle rounded-sm" />
                </div>
              </div>
            ) : history.length === 0 && status === ConnectionStatus.CONNECTED && (
               <div className="w-full text-center space-y-4">
                  <p className="text-zinc-400 text-2xl font-light italic animate-pulse">
                    "I'm ready, Master E."
                  </p>
                  <p className="text-[10px] text-blue-900 uppercase tracking-[0.5em] font-black">Waiting for Command</p>
               </div>
            )}
          </div>
        </div>
            ...
            >
              {status === ConnectionStatus.CONNECTED ? (
                ...
                <>
                  ...
                  <span className="text-[10px] font-black tracking-[0.3em] uppercase">Awaken</span>
                </>
              )}
            </button>
          </div>
